...
def find_replays(pokeSearch,meta,replay_total=100,filters={ "teamused":False, "rating" : 0, "winner": False,"min_upload": 0, "max_upload" : 0, "allow_duplicate_players" : False}):
    ...
    # Day Filter
    if (filters["max_upload"]>0):
        search_filter.append((df['uploadtime'] >= filters["min_upload"]) & (df['uploadtime'] <= filters["max_upload"]))
    else:
        search_filter.append(df['uploadtime'] >= filters["min_upload"])
...
def load_replay():
    ...
    filter_date_enabled = request.args.get('filter_date_enabled') == 'true'
    ...
    filter_day_min = request.args.get('filter_day_min')
    filter_day_max = request.args.get('filter_day_max')

    if (filter_date_enabled == False):
        filter_day_min = 0
        filter_day_max = 0

    filters={ "teamused":filter_battleused, "rating" : 0, "winner": filter_winner,"min_upload": int(filter_day_min), "max_upload" : int(filter_day_max) }
