$(document).ready(function() {
    ...
    function updateReplays(replays,format) {
        ...
        replays.forEach(function(replay) {
            ...
            if (bo3) {
                ...
                li_item+= '<button class = "team-container" onclick="openReplayModal(\''+replay[0]+'\', \''+replay[3][0]+' vs '+replay[3][1]+'\')">';
            }
    ...
    function loadTeamRankings() {
        ...
        $('#rankings-format-name').text(format);
...
// Replay Modal
var currentBo3Data = null; // { matches: [id1, id2, id3], title: "P1 vs P2", activeGame: 0 }
...
function openReplayModal(replayId, title, bo3Matches, gameIndex) {
    ...
    iframe.src = '/watch/' + replayId;
    modal.classList.add('active');
    document.body.style.overflow = 'hidden';

    // Build game navigation for Bo3 sets
...
function switchBo3Game(gameIndex) {
    ...
    // Use postMessage to swap the replay without a full iframe reload
...
// Listen for replay switch requests from the iframe (e.g. "Game 2 of 3" links)
window.addEventListener('message', function(e) {
    if (e.data && e.data.type === 'switchReplay' && e.data.replayId) {
        var newId = e.data.replayId;

        // If we have Bo3 data, check if this replay is one of the games and update active state
        if (currentBo3Data) {
